...
pub fn current_cumulative_prices(&self, pair: AccountId) -> Result<(u128, u128, u32), Error> {
    ...
    if self.data().block_timestamp_last != block_timestamp {
        let mut time_elapsed: u32 = block_timestamp - self.data().block_timestamp_last;
        price_0_cumulative += <u128>::from(
            fixed_point
                .fraction(self.data().reserve_1, self.data().reserve_0)?
                .x,
        ) * time_elapsed;
        price_1_cumulative += <u128>::from(
            fixed_point
                .fraction(self.data().reserve_0, self.data().reserve_1)?
                .x,
        ) * time_elapsed;
    }
