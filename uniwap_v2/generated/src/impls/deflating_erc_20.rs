...
impl<T: Storage<Data>> DeflatingERC20 for T {
    ...
    fn permit(
        ...
        if !(recovered_address != ZERO_ADDRESS.into() && recovered_address == owner) {
            return Err(Error::Custom(String::from("INVALID_SIGNATURE")))
        };
...
impl<T: Storage<Data>> Internal for T {
    ...
    default fn _transfer(
        ...
        let mut burn_amount: u128 = value / 100;
        self._burn(from, burn_amount)?;
        let mut transfer_amount: u128 = value.sub(burn_amount)?;
        self.data().balance_of.insert(
            &(from),
            &(self
                .data()
        ...
        self._emit_transfer(from, to, transfer_amount);
        Ok(())
    }
